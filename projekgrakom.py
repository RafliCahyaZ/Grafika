...
class PaintApp:
    ...
    def build_toolbar(self):
        toolbar = tk.Frame(self.root, bg="#242424", height=132)
        ...
        self.add_group(toolbar, "Image", [
            ("🖼", "Buka", self.open_image),
            ("⟲", "Putar kiri", lambda: self.rotate_canvas("left")),
            ("⟳", "Putar kanan", lambda: self.rotate_canvas("right")),
        ])

        self.add_group(toolbar, "Tools", [
            ("✏", "Brush", lambda: self.set_tool("brush")),
            ("🧽", "Eraser", lambda: self.set_tool("eraser")),
            ("🪣", "Fill layer", lambda: self.set_tool("fill")),
            ("💧", "Pipet", lambda: self.set_tool("picker")),
            ("A", "Text", lambda: self.set_tool("text")),
        ])

        brush_group = tk.Frame(toolbar, bg="#242424", padx=10)
        brush_group.pack(side=tk.LEFT, fill=tk.Y)
        tk.Label(brush_group, text="Ukuran", bg="#242424", fg="white").pack(anchor="w")
        tk.Scale(brush_group, from_=1, to=60, orient=tk.HORIZONTAL, variable=self.brush_size, bg="#242424", fg="white", troughcolor="#3a3a3a", highlightthickness=0, length=150).pack()
        tk.Label(brush_group, text="Brushes", bg="#242424", fg="#cfcfcf", font=("Segoe UI", 9)).pack(side=tk.BOTTOM)

        shape_group = tk.Frame(toolbar, bg="#242424", padx=10)
        ...
        for i, (label, shape) in enumerate(shape_buttons):
            tk.Button(grid, text=label, width=3, bg="#303030", fg="white", relief=tk.FLAT, command=lambda s=shape: self.set_shape(s)).grid(row=0, column=i, padx=2, pady=2)
        tk.Label(shape_group, text="Algoritma", bg="#242424", fg="white").pack(anchor="w")
        self.algorithm_combo = ttk.Combobox(shape_group, state="readonly", width=16, values=["normal", "DDA Line", "Bresenham Line", "Midpoint Circle", "Midpoint Ellipse"])
        self.algorithm_combo.set("normal")
        self.algorithm_combo.pack()
        self.algorithm_combo.bind("<<ComboboxSelected>>", self.change_algorithm)
        tk.Label(shape_group, text="Shapes & Algorithm", bg="#242424", fg="#cfcfcf", font=("Segoe UI", 9)).pack(side=tk.BOTTOM)

        color_group = tk.Frame(toolbar, bg="#242424", padx=10)
        ...
        tk.Label(color_group, text="Colours", bg="#242424", fg="#cfcfcf", font=("Segoe UI", 9)).pack(side=tk.BOTTOM)

        transform_group = tk.Frame(toolbar, bg="#242424", padx=10)
        ...
        for i, (label, command, tip) in enumerate(transform_buttons):
            ...
            self.create_tooltip(btn, tip)
        tk.Label(transform_group, text="Transform", bg="#242424", fg="#cfcfcf", font=("Segoe UI", 9)).pack(side=tk.BOTTOM)

        anim_group = tk.Frame(toolbar, bg="#242424", padx=10)
        anim_group.pack(side=tk.LEFT, fill=tk.Y)
        tk.Button(anim_group, text="Bounce", bg="#303030", fg="white", relief=tk.FLAT, command=self.start_bounce_animation).pack(pady=4, fill=tk.X)
        tk.Button(anim_group, text="Rotate", bg="#303030", fg="white", relief=tk.FLAT, command=self.start_rotate_animation).pack(pady=4, fill=tk.X)
        tk.Button(anim_group, text="Stop", bg="#8b1e1e", fg="white", relief=tk.FLAT, command=self.stop_animation).pack(pady=4, fill=tk.X)
        tk.Label(anim_group, text="Animation", bg="#242424", fg="#cfcfcf", font=("Segoe UI", 9)).pack(side=tk.BOTTOM)

        layer_group = tk.Frame(toolbar, bg="#242424", padx=10)
        layer_group.pack(side=tk.LEFT, fill=tk.Y)
        tk.Button(layer_group, text="+ Layer", bg="#303030", fg="white", relief=tk.FLAT, command=self.add_layer).pack(pady=6)
        self.layer_combo = ttk.Combobox(layer_group, state="readonly", width=14)
        self.layer_combo.pack()
        self.layer_combo.bind("<<ComboboxSelected>>", self.change_layer)
        tk.Button(layer_group, text="👁 Show/Hide", bg="#303030", fg="white", relief=tk.FLAT, command=self.toggle_layer).pack(pady=4)
        tk.Label(layer_group, text="Layers", bg="#242424", fg="#cfcfcf", font=("Segoe UI", 9)).pack(side=tk.BOTTOM)
        self.refresh_layer_list()

    def add_group(self, parent, title, items):
        group = tk.Frame(parent, bg="#242424", padx=10)
        group.pack(side=tk.LEFT, fill=tk.Y)
        box = tk.Frame(group, bg="#242424")
        box.pack(pady=10)
        for i, (symbol, tooltip, command) in enumerate(items):
            btn = tk.Button(box, text=symbol, command=command, width=3, height=1, bg="#303030", fg="white", relief=tk.FLAT, font=("Segoe UI", 12))
            btn.grid(row=i // 3, column=i % 3, padx=2, pady=2)
            self.create_tooltip(btn, tooltip)
        tk.Label(group, text=title, bg="#242424", fg="#cfcfcf", font=("Segoe UI", 9)).pack(side=tk.BOTTOM)
    ...
    def build_workspace(self):
        ...
        panel = tk.LabelFrame(left_panel, text="Ukuran & Presisi", bg="#202020", fg="white", padx=10, pady=10)
        ...
        self.width_entry.pack(fill=tk.X, pady=3)
        ...
        self.height_entry.pack(fill=tk.X, pady=3)

        tk.Button(panel, text="Terapkan Ukuran Kanvas", bg="#0d6efd", fg="white", relief=tk.FLAT, command=self.apply_canvas_size).pack(fill=tk.X, pady=8)

        tk.Label(panel, text="Teks", bg="#202020", fg="white").pack(anchor="w")
        tk.Entry(panel, textvariable=self.text_value, bg="#303030", fg="white", insertbackground="white").pack(fill=tk.X, pady=3)

        tk.Label(panel, text="Opasitas", bg="#202020", fg="white").pack(anchor="w")
        tk.Scale(panel, from_=5, to=100, orient=tk.HORIZONTAL, variable=self.opacity, bg="#202020", fg="white", troughcolor="#3a3a3a", highlightthickness=0).pack(fill=tk.X)

        tk.Checkbutton(panel, text="Tampilkan Grid", variable=self.show_grid, command=self.full_render, bg="#202020", fg="white", selectcolor="#303030", activebackground="#202020").pack(anchor="w", pady=4)
        tk.Button(panel, text="Simpan PNG", bg="#198754", fg="white", relief=tk.FLAT, command=self.save_image).pack(fill=tk.X, pady=4)
        tk.Button(panel, text="Bersihkan", bg="#dc3545", fg="white", relief=tk.FLAT, command=self.clear_canvas).pack(fill=tk.X, pady=4)

        object_panel = tk.LabelFrame(left_panel, text="Info Objek / Area", bg="#202020", fg="white", padx=10, pady=10)
        object_panel.pack(fill=tk.X, pady=10)
        ...
        tk.Button(object_panel, text="Ambil Area Seleksi", bg="#303030", fg="white", relief=tk.FLAT, command=self.capture_selection).pack(fill=tk.X, pady=4)
        tk.Button(object_panel, text="Hapus Seleksi", bg="#303030", fg="white", relief=tk.FLAT, command=self.clear_selection).pack(fill=tk.X, pady=4)

        algo_panel = tk.LabelFrame(left_panel, text="Koordinat Manual", bg="#202020", fg="white", padx=10, pady=10)
        algo_panel.pack(fill=tk.X)
        ...
        tk.Button(algo_panel, text="Gambar dari Angka", bg="#0d6efd", fg="white", relief=tk.FLAT, command=self.draw_from_numbers).pack(fill=tk.X, pady=6)
    ...
    def build_statusbar(self):
        status = tk.Frame(self.root, bg="#222222", height=36)
    ...
    def draw_grid_items(self):
        step = max(1, int(25 * self.zoom))
        w = int(self.canvas_width * self.zoom)
        h = int(self.canvas_height * self.zoom)
        ox, oy = self.canvas_offset_x, self.canvas_offset_y
        for x in range(0, w + 1, step):
            self.canvas.create_line(ox + x, oy, ox + x, oy + h, fill="#dddddd", stipple="gray50", tags="grid")
        for y in range(0, h + 1, step):
            self.canvas.create_line(ox, oy + y, ox + w, oy + y, fill="#dddddd", stipple="gray50", tags="grid")
    ...
    def on_mouse_down(self, event):
        ...
        if self.tool == "fill":
            self.fill_layer(self.primary_color)
            self.save_history()
            self.full_render()
            return
        if self.tool == "picker":
    ...
    def draw_point_to_image(self, x, y):
        draw = ImageDraw.Draw(self.active_image())
        color = (255, 255, 255, 255) if self.tool == "eraser" else self.hex_to_rgba(self.primary_color)
    ...
    def start_rotate_animation(self):
        ...
        if self.selection:
            ...
            x1, y1 = 120, 120
        self.anim_obj = {"image": obj, "x": x1, "y": y1, "mode": "rotate"}
        self.anim_angle = 0
        self.animation_running = True
        self.animate()
    ...
    def animate(self):
        ...
        display = render_obj.resize((max(1, int(render_obj.width * self.zoom)), max(1, int(render_obj.height * self.zoom))), Image.Resampling.BILINEAR)
        self.anim_tk = ImageTk.PhotoImage(display)
        cx, cy = self.image_to_canvas_coords(self.anim_obj["x"], self.anim_obj["y"])
    ...
    def show_file_menu(self):
        self.popup_menu([("Open Image", self.open_image), ("Save PNG", self.save_image), ("-", None), ("Clear Canvas", self.clear_canvas)])
    ...
    def show_animation_menu(self):
        self.popup_menu([("Bounce Animation", self.start_bounce_animation), ("Rotate Animation", self.start_rotate_animation), ("Stop Animation", self.stop_animation)])
